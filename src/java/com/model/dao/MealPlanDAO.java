...
public class MealPlanDAO {
    ...
    public double getMenuGrowth() {
        double thisMonth = 0;
        double lastMonth = 0;

        String sqlThis = "SELECT COUNT(*) FROM daily_menu WHERE MONTH(Menu_date) = MONTH(CURRENT_DATE()) AND YEAR(Menu_date) = YEAR(CURRENT_DATE())";
        String sqlLast = "SELECT COUNT(*) FROM daily_menu WHERE MONTH(Menu_date) = MONTH(CURRENT_DATE() - INTERVAL 1 MONTH) AND YEAR(Menu_date) = YEAR(CURRENT_DATE() - INTERVAL 1 MONTH)";
